<template>
  <div class="login-form white" color="primary">
    ...
    <div class="
        ...
        justify-center
      ">
      <v-progress-circular v-if="loginWait" :size="100" :width="5" class="login-form__loader" color="primary"
        indeterminate />

      <template v-else>

        <!-- logo -->
        <img v-if="showLogo" class="mb-2" :src="require(`@/assets/images/${logo}`)">
        ...
        <v-form v-model="valid" class="login-form__form" ref="form" lazy-validation @submit.prevent>
          <v-text-field :label="$t('global.login.login')" v-model="user" :rules="loginRules" required></v-text-field>
          <v-text-field :label="$t('global.login.password')" v-model="password" :rules="passwordRules" :counter="30"
            required :append-icon="passAppendIcon" @click:append="() => (passwordHidden = !passwordHidden)"
            :type="passTextFieldType" class="mb-5"></v-text-field>
          <v-btn block @click="loginAttempt()" :disabled="!valid" class="primary white--text">
  ...
  computed: {
    ...
    loginRules() {
      return [
        ...
        v => this.alphanumericRegex.test(v) || this.$t('global.login.incorrectLogin'),
      ]
    },
    passwordRegex() {
